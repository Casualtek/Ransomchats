...
def parse_html_to_json(html_file, output_file):
    ...
    # Find all chat items
    chat_items = soup.find_all('div', class_='msg')

    # Extract party and message details for each chat item
    messages = []
    for item in chat_items:
        if 'right-msg' in item['class']:
            party = 'Victim'
        elif 'left-msg' in item['class']:
            party = 'Avos'

        datetime_div = item.find('div', class_='msg-info-time')
        datetime = datetime_div.get_text().strip()
        datetime_div.decompose()
        
        name_div = item.find('div', class_='msg-info-name')
        name_div.decompose()
