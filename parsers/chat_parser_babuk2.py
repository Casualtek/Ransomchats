...
def parse_html_to_json(html_file, output_file):
    ...
    for item in chat_items:
        message = item.find('div', class_='message-from')
        if message is not None:
            party = 'Victim'
        
        message = item.find('div', class_='message-to')
        if message is not None:
            party = 'Babuk'

        # Extract message content and timestamp
        date_div = item.find('time', class_='message-time')
        datetime = date_div['datetime']
        
        # Trim content up to the first occurrence of '<div '
        content_div = item.find('div', class_='message-text')
