...
def parse_html_to_json(html_file, output_file):
    ...
    # Find all chat items
    chat_items = soup.find_all('div', class_='message')

    # Extract party and message details for each chat item
    messages = []
    for item in chat_items:
        message = item.find('div', class_='message-bot')
        if message is not None:
            party = 'Victim'
        
        message = item.find('div', class_='message-support')
        if message is not None:
            party = 'Ranzy'

        datetime = item.find('div', class_='message-date').get_text().strip()
        content = item.find('div', class_='message-text').get_text().strip()
