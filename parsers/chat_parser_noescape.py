...
def parse_html_to_json(html_file, output_file):
    ...
    # Find all chat items
    chat       = soup.find_all('div', class_='message-box')

    # Extract party and message details for each chat item
    messages = []
    for item in chat:
        party_info = item.find('div', class_='sender')
        if 'You' == party_info.get_text().strip():
            party = 'Victim'
        elif 'Support' == party_info.get_text().strip():
            party = 'NoEscape'
        
        message = item.find('div', class_='message-body')
        message_text = message.find('pre')
        message_time = message.find('div', class_='message-time')

        message = {
            'party': party,
            'content': message_text.get_text().strip(),
            'timestamp': message_time.get_text().strip()
        }
