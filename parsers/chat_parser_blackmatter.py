...
def parse_html_to_json(html_file, output_file):
    ...
    # Find all chat items
    chat_items = soup.find_all('div', class_='container-msg')

    # Extract party and message details for each chat item
    messages = []
    for item in chat_items:
        if 'right' in item['class']:
            party = 'Victim'
        elif 'left' in item['class']:
            party = 'BlackMatter'

        datetime_tag = item.find('small')
        datetime = datetime_tag.get_text().strip()
        
        content_div = item.find('div', class_='text-white')
