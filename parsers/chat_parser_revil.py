...
def parse_html_to_json(html_file, output_file):
    ...
    for item in chat_items:
        if 'victim' in item['class']:
            party = 'Victim'
        elif 'support' in item['class']:
            party = 'REvil'

        datetime_div = item.find('div', class_='chat-timestamp')
        datetime = datetime_div.get_text().strip()
        
        content_div = item.find('div', class_='chat-message-body')
        if content_div is not None:
            content = content_div.get_text().strip()
        else:
            content = ''
            
        #attachements = item.find('div', class_='attachment-item')
